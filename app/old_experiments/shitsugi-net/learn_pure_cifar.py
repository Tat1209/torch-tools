...
# Model.load_ckpt_network(network, f"pt_default.ckpt")
network.fc = torch.nn.Linear(network.fc.in_features, 100)
...
for e in range(epochs):
    model.train_1epoch(pr.train(tr.gen))
    model.val_1epoch(pr.cal_val(tr.gen))
