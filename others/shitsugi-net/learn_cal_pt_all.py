...
#今の終わったら実行したい．時間絶対足らんけど
for j in range(3):
    for i in range(5):
        model_name = f"cal_pt_{j}_{i}times"
        ...
        network = net()
        Model.load_ckpt_network(network, f"pt_{j}_{i}times.ckpt")
...
# for i in range(5):
#     model_name = f"cal_pt_{i}times"

#     tr = Trans(info={'mean': [0.5070751309394836, 0.48654884099960327, 0.44091784954071045], 'std': [0.2673342823982239, 0.2564384639263153, 0.2761504650115967]})

#     network = net()
#     Model.load_ckpt_network(network, f"pt_{i}times.ckpt")
#     network.fc = torch.nn.Linear(network.fc.in_features, 101)
